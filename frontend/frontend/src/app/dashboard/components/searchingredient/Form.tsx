...
export default function Form({ ingredients }: Props) {
  ...
  async function RemoveIngredient(id: number) {
    ...
    await api.delete("/deleteIngredient", {
      data: { id },
      headers: {
  ...
  return (
    <section className={style.main_section}>
      <div className={style.main_div}>
        ...
        <Link className={style.btnSearch} href="/dashboard/ingredients">
            ...
            .map((ingredient, index) => (
              <li key={index} className={style.ingredient_card}>
                ...
                <div className={style.ingredient_info}>
                  <p>
                    <strong>Preço unitário:</strong> R$ {ingredient.unitPrice.toFixed(2)} por{" "}
                    {ingredient.unitConversion}
                  </p>
                  <p>
                    <strong>Medida:</strong> {ingredient.totalUnit} {ingredient.unitConversion}
                  </p>
                  <p>
                  ...
                  <button
                    ...
                    onClick={() => handleDetailIngredient(ingredient.id)}
                  >
                    Editar
                  </button>
                </div>
